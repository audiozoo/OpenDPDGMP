"""
Block-based GMP Digital Pre-Distortion with Indirect Learning Architecture.

Instead of processing the entire input signal in one least-squares solve,
this script segments the signal into blocks of BLOCK_SIZE samples and
updates the GMP coefficients on each block.  Coefficients are damped
(blended with the previous estimate) to ensure stable convergence.

The flow for each block:
  1. Extract the block from the input signal
  2. Pass it through the PA (without DPD for the first block, with DPD after)
  3. Identify new GMP coefficients from the block's (input, output) pair
  4. Blend with previous coefficients:  w = α·w_new + (1-α)·w_old
  5. Record NMSE and ACLR metrics

After all blocks are processed, a summary plot shows the PSD at the final
iteration and the NMSE/ACLR convergence across blocks.
"""
...
# ===================================================================
# Main — block-based DPD identification
# ===================================================================
...
def main():
    ...
    print(f"Block-based GMP DPD identification")
    ...
    for b in range(n_blocks):
        ...
        if w is None:
            ...
            # Re-identify from the predistorted block
    ...
    # ------------------------------------------------------------------
    # Plots: 3-panel — PSD, NMSE vs block, ACLR vs block
    # ------------------------------------------------------------------
    nperseg_psd = 8192
    ...
    ax_psd.set_title(f'{n_carriers}× WCDMA {carrier_bw_mhz} MHz — '
                     f'Block-based GMP DPD ({BLOCK_SIZE} samples/block)',
                     fontsize=11)
    ...
    fig.tight_layout()
    output_file = 'gmp_dpd_block_convergence.png'
