"""
Block-based GMP DPD with two-phase damping and coefficient update guard.

V3: Uses a higher damping factor (α=0.5) for the first 8 accepted
coefficient updates to converge quickly, then switches to a lower
damping factor (α=0.1) for finer refinement.

The coefficient update guard from V2 is retained: if the coefficient
NMSE exceeds 0 dB, the update is rejected and the previous coefficients
are kept.
"""
...
def main():
    ...
    target_rms      = 0.22
    damping_initial = 0.5   # α for first N updates (fast convergence)
    damping_fine    = 0.1   # α for remaining updates (fine refinement)
    damping_switch  = 8     # switch after this many accepted updates
    COEFF_NMSE_THRESHOLD = 0.0  # dB — reject updates above this

    pa_K, pa_Q = 5, 2
    cfg = GMPConfig(Ka=7, La=5, Kb=3, Lb=5, Mb=2, Kc=3, Lc=5, Mc=1)

    print(f"Block-based GMP DPD identification (v3 — two-phase damping)")
    ...
    print(f"  Damping: α={damping_initial} for first {damping_switch} "
          f"updates, then α={damping_fine}")
    ...
    block_damping = []    # damping α used per block
    n_rejected = 0
    ...
    for b in range(n_blocks):
        ...
        # Select damping factor based on how many updates have been accepted
        damping = (damping_initial if coeff_update_idx < damping_switch
                   else damping_fine)
        ...
        if b % 10 == 0 or b == n_blocks - 1:
            ...
            print(f"  Block {b:3d}/{n_blocks} (α={damping:.1f}): "
    ...
    block_damping_arr = np.array(block_damping)
    ...
    ax['psd'].set_title(f'{n_carriers}× WCDMA {carrier_bw_mhz} MHz — '
                     f'Block DPD v3 ({BLOCK_SIZE} samples/block, '
                     f'α={damping_initial}→{damping_fine} after '
                     f'{damping_switch} updates)', fontsize=11)
    ...
    fig_s.tight_layout()
    fig_s.savefig('gmp_dpd_block_v3_convergence.png', dpi=150)
    print(f"\nStatic plot saved to gmp_dpd_block_v3_convergence.png")
    plt.close(fig_s)
    ...
    gif_file = 'gmp_dpd_block_v3_convergence.gif'
