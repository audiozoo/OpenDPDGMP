"""
Block-based GMP DPD with coefficient update guard — 5G NR OFDM signal.

Adapted from gmp_dpd_block_v2.py: replaces 3-carrier WCDMA with a single
20 MHz NR carrier (SCS = 30 kHz, 51 PRBs, 612 active subcarriers, QPSK).

Same DPD engine: ILA with damping (α=0.5) and coefficient-NMSE guard
that rejects updates exceeding 0 dB relative change.
"""
...
# ===================================================================
# 5G NR OFDM signal generation
# ===================================================================

def generate_nr_ofdm(n_samples, bw_mhz=20, scs_khz=30, fs=61.44e6):
    """
    Single-carrier 5G NR–like OFDM signal.
    ...
    bw_mhz : float
        Channel bandwidth in MHz (determines filter edges).
    scs_khz : float
    ...
    np.ndarray (complex128)
        Baseband IQ signal of length n_samples.

    3GPP TS 38.104 Table 5.3.2-1: 20 MHz / 30 kHz SCS → 51 PRBs.
    """
    scs = scs_khz * 1e3
    n_fft = int(round(fs / scs))                   # 2048 at 61.44 MHz
    n_prb = 51                                       # 3GPP for 20 MHz / 30 kHz
    n_sc = n_prb * 12                                # 612 active subcarriers

    # Normal CP for μ=1 at native rate (2048-FFT): 144 / 160 samples
    cp_normal = int(round(144 * n_fft / 2048))
    cp_extended = int(round(160 * n_fft / 2048))

    symbols_per_slot = 14
    n_ext_per_slot = 2                               # symbols 0 and 7
    ...
    for _slot in range(n_slots):
        for sym_idx in range(symbols_per_slot):
            ...
            # QPSK on every active subcarrier
            data = (np.sign(np.random.randn(n_sc))
                    + 1j * np.sign(np.random.randn(n_sc))) / np.sqrt(2)

            freq = np.zeros(n_fft, dtype=np.complex128)
            freq[1:half_sc + 1] = data[:half_sc]          # positive freqs
            freq[n_fft - half_sc:] = data[half_sc:]        # negative freqs
    ...
    occupied_bw = n_sc * scs                         # 18.36 MHz
    pb = occupied_bw / 2 + scs                       # passband edge + 1 SCS guard
    sb = bw_mhz * 1e6 / 2                            # stopband at channel edge (10 MHz)
...
def main():
    ...
    scs_khz         = 30
    bw_mhz          = 20
    fs              = 61.44e6         # 2048-pt FFT × 30 kHz SCS (native μ=1)
    channel_bw_hz   = bw_mhz * 1e6   # 20 MHz
    adjacent_offset = channel_bw_hz   # NR adjacent channel at ±20 MHz

    BLOCK_SIZE      = 65536            # ~1.07 ms ≈ 2 NR slots (28 symbols)
    N_SIGNAL        = 2_000_000
    n_blocks        = N_SIGNAL // BLOCK_SIZE

    target_rms      = 0.15            # OFDM PAPR ~3 dB > WCDMA; back off from 0.22
    damping         = 0.3             # conservative blend to avoid block overfitting
    COEFF_NMSE_THRESHOLD = -3.0       # dB — moderate guard

    cfg = GMPConfig(Ka=9, La=7, Kb=5, Lb=7, Mb=2, Kc=5, Lc=7, Mc=1)

    print("Block-based GMP DPD identification (NR OFDM — with update guard)")
    print(f"  Signal: 1× NR {bw_mhz} MHz, SCS={scs_khz} kHz, "
          f"51 PRBs (612 sc), fs={fs/1e6:.2f} MHz")
    ...
    # ------------------------------------------------------------------
    print("Generating NR OFDM signal...", flush=True)
    x_full_c = generate_nr_ofdm(N_SIGNAL, bw_mhz=bw_mhz, scs_khz=scs_khz,
                                 fs=fs)
    ...
    psd_xlim = (-30, 30)  # ±30 MHz covers adjacent channels at ±20 MHz
    ...
    # Main channel shading
    ax['psd'].axvspan(-bw_mhz / 2, bw_mhz / 2,
                      color='green', alpha=0.06, label=f'{bw_mhz} MHz channel')
    ...
    ax['psd'].set_xlim(*psd_xlim); ax['psd'].set_ylim(-80, 5)
    ax['psd'].set_xlabel('Frequency (MHz)', fontsize=12)
    ax['psd'].set_ylabel('PSD (dB, normalized)', fontsize=12)
    ax['psd'].set_title(f'NR {bw_mhz} MHz SCS={scs_khz} kHz — '
                        f'Block DPD ({BLOCK_SIZE} samples/block, '
    ...
    fig_s.tight_layout()
    fig_s.savefig('gmp_dpd_block_nr_convergence.png', dpi=150)
    print(f"\nStatic plot saved to gmp_dpd_block_nr_convergence.png")
    plt.close(fig_s)
    ...
    a_psd.axvspan( adjacent_offset / 1e6 - bw_mhz / 2,
                   adjacent_offset / 1e6 + bw_mhz / 2,
                  color='orange', alpha=0.04)
    a_psd.set_xlim(*psd_xlim); a_psd.set_ylim(-80, 5)
    ...
    def update(frame_idx):
        ...
        a_psd.set_title(f'NR {bw_mhz} MHz SCS={scs_khz} kHz — '
    ...
    gif_file = 'gmp_dpd_block_nr_convergence.gif'
