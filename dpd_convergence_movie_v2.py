...
V2: Uses **coefficient damping** to prevent the ILA limit-cycle oscillation
seen in V1.  At each iteration the newly identified coefficients are blended
with the previous ones:

    w_{k+1} = α · w_new + (1 - α) · w_k       (α = 0.5)

This smooths the update and produces monotone convergence.

Each frame plots the PSD of the PA output at that iteration, all normalized
to 0 dB peak.  Saved as a GIF.
"""
...
def main():
    ...
    damping = 0.5  # blend factor: w = α·w_new + (1-α)·w_old
    ...
    for it in range(1, n_iterations + 1):
        ...
        # Re-identify and damp: blend new coefficients with previous
        w_new = identify_gmp(x_dpd, y_dpd, cfg, target_gain)
    ...
    output_file = 'dpd_convergence_v2.gif'
    ...
    fig2.tight_layout()
    fig2.savefig('dpd_convergence_summary_v2.png', dpi=150)
    print("Summary plot saved to dpd_convergence_summary_v2.png")
