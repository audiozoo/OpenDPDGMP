"""
Block-based GMP DPD — 5G NR 100 MHz **unfiltered** OFDM, multi-rate.

Signal chain:
  1. Generate unfiltered OFDM at fs_sig = 122.88 MHz (4096-pt FFT × 30 kHz SCS)
     No channel filter — raw sinc sidelobes preserved.
...
def generate_nr_ofdm(n_samples, bw_mhz=100, scs_khz=30, fs=122.88e6):
    """
    Single-carrier 5G NR OFDM signal at the native DAC rate — **unfiltered**.
    ...
    No channel filter is applied; the raw OFDM signal retains its
    inherent sinc sidelobes from rectangular symbol windowing.
    """
    scs = scs_khz * 1e3
...
def main():
    ...
    print("Block-based GMP DPD — NR 100 MHz unfiltered OFDM, multi-rate")
    ...
    # ------------------------------------------------------------------
    # OFDM spectral shape plot (no channel filter in this version)
    # ------------------------------------------------------------------
    print("Plotting OFDM spectral shape ...", flush=True)
    ...
    fig_filt, ax_ofdm = plt.subplots(1, 1, figsize=(14, 6))
    ...
    ax_ofdm.set_xlabel('Frequency (MHz)', fontsize=12)
    ax_ofdm.set_ylabel('Magnitude (dB)', fontsize=12)
    ax_ofdm.set_title(
        f'Unfiltered OFDM Spectral Shape (sinc sidelobes)\n'
    ...
    fig_filt.savefig('gmp_dpd_block_nr100_v4_ofdm_spectrum.png', dpi=150)
    print(f"  Spectral shape plot saved to gmp_dpd_block_nr100_v4_ofdm_spectrum.png")
    plt.close(fig_filt)
    ...
    ax['psd'].set_title(
        f'NR {bw_mhz} MHz SCS={scs_khz} kHz (unfiltered OFDM) — '
        f'PSD at {fs_psd/1e6:.0f} MHz\n'
    ...
    fig_s.tight_layout()
    fig_s.savefig('gmp_dpd_block_nr100_v4_convergence.png', dpi=150)
    print(f"\nStatic plot saved to gmp_dpd_block_nr100_v4_convergence.png")
    plt.close(fig_s)
    ...
    def update(frame_idx):
        ...
        a_psd.set_title(
            f'NR {bw_mhz} MHz SCS={scs_khz} kHz (unfiltered) — '
    ...
    gif_file = 'gmp_dpd_block_nr100_v4_convergence.gif'
