"""
Block-based GMP DPD with coefficient update guard — 5G NR 100 MHz OFDM.

Single carrier 100 MHz, SCS = 30 kHz, 273 PRBs (3276 active subcarriers).
fs = 368.64 MHz (3 × 122.88 MHz) so that the NR adjacent channel at
±100 MHz (edges at ±150 MHz) fits within the ±184.32 MHz Nyquist band.

Same DPD engine as gmp_dpd_block_nr.py: ILA with damping (α=0.3)
and coefficient-NMSE guard.
"""
...
# ===================================================================
# 5G NR OFDM signal generation (parameterised for any BW / SCS)
# ===================================================================

# 3GPP TS 38.104 Table 5.3.2-1 — max PRBs for SCS = 30 kHz
NR_PRB_TABLE_SCS30 = {
...
def generate_nr_ofdm(n_samples, bw_mhz=100, scs_khz=30, fs=368.64e6):
    """
    ...
    """
    scs = scs_khz * 1e3
    n_fft = int(round(fs / scs))
    n_prb = NR_PRB_TABLE_SCS30[int(bw_mhz)]
    n_sc = n_prb * 12

    # CP lengths scaled from the 2048-FFT reference
    ...
    occupied_bw = n_sc * scs
    pb = occupied_bw / 2 + scs
    sb = bw_mhz * 1e6 / 2
...
def main():
    ...
    scs_khz         = 30
    bw_mhz          = 100
    fs              = 368.64e6        # 12288-pt FFT × 30 kHz; Nyquist 184.32 MHz
    channel_bw_hz   = bw_mhz * 1e6   # 100 MHz
    adjacent_offset = channel_bw_hz   # NR adj. ch. at ±100 MHz (edges ±150 MHz)

    n_prb           = NR_PRB_TABLE_SCS30[bw_mhz]
    n_sc            = n_prb * 12

    BLOCK_SIZE      = 131072          # ~0.36 ms ≈ 8 OFDM symbols
    N_SIGNAL        = 2_000_000
    n_blocks        = N_SIGNAL // BLOCK_SIZE

    target_rms      = 0.15            # OFDM PAPR headroom
    damping         = 0.3
    COEFF_NMSE_THRESHOLD = -3.0       # dB

    cfg = GMPConfig(Ka=9, La=7, Kb=5, Lb=7, Mb=2, Kc=5, Lc=7, Mc=1)

    print("Block-based GMP DPD identification "
          "(NR 100 MHz OFDM — with update guard)")
    print(f"  Signal: 1× NR {bw_mhz} MHz, SCS={scs_khz} kHz, "
          f"{n_prb} PRBs ({n_sc} sc), fs={fs/1e6:.2f} MHz")
    ...
    # ------------------------------------------------------------------
    print("Generating NR 100 MHz OFDM signal...", flush=True)
    ...
    fig_s.tight_layout()
    fig_s.savefig('gmp_dpd_block_nr100_convergence.png', dpi=150)
    print(f"\nStatic plot saved to gmp_dpd_block_nr100_convergence.png")
    plt.close(fig_s)
    ...
    gif_file = 'gmp_dpd_block_nr100_convergence.gif'
